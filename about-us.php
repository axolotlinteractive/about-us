...
add_action( 'admin_init', '\AboutUs\has_word_wrap' );
...
function checkPHPVersion($minimalRequiredPHPVersion) {
    if (version_compare(phpversion(), $minimalRequiredPHPVersion) < 0) {
        add_action('admin_notices', 'noticePhpVersionWrong');
        return false;
    }
    return true;
}
...
if (checkPHPVersion($minimalRequiredPHPVersion)) {
